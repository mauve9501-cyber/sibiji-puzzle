/*
 * 십이지 그림 조각 퍼즐 — 십장생플러스 브랜드 빌드
 * puzzle-source.html (아티팩트 원본) + supabase-config.json → index.html
 * 퍼즐이 업데이트되면 puzzle-source.html 만 교체하고 `node build.js` 재실행.
 */
const fs = require("fs");
...
@media (prefers-color-scheme:dark){.sjs-brandbar,.sjs-followband{background:#06285c}}
...
const BRANDBAR = `
  <div class="sjs-brandbar">
    <div class="bb-brand">
      ...
      <span class="bb-name"><strong>십장생플러스</strong><em>십이지 · 오방색 그림 조각 퍼즐</em></span>
    </div>
...
console.log('index.html 생성 완료 ('+ (html.length/1024/1024).toFixed(2) +' MB) · supabase='+(sbUrl?'설정됨':'로컬폴백'));
